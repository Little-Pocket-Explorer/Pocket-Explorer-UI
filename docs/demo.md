...
1. Open Pocket Explorer. On first launch choose device language or one of the ten supported languages. The profile language option changes the saved choice later.
...
4. Reveal the card and turn it over to read your words. Show the generated illustration when it is ready. If it is still waiting, show that the saved words remain usable and continue with an earlier saved illustrated card, identifying it as an earlier exploration. A field photo remains private. The card can be revisited after relaunch.
...
Two minutes is the rehearsal target, not a measured guarantee. Provider duration varies. A recording supports a presentation but does not replace real-device verification. Preserve actual input on failures and show the recovery message. A localhost URL is not a public sharing link.
