...
1. Create a feature branch and open a pull request. Native regression, live Cloudflare sharing integration, coverage and workflow lint run without signing secrets.
2. Update ios/fastlane/TESTFLIGHT_NOTES.md when the test instructions change.
3. Merge into main. Changes to iOS, native shared resources or CI trigger the workflow. After every check passes, the release job signs, uploads, waits for Apple processing and assigns Hackathon Internal.
...
Native tests create an isolated iPhone SE simulator with iOS 26.5 and test the deployed Cloudflare sharing service. They retain ad hoc simulator signing for Keychain access and require at least 80% app line coverage. Backend CI owns the web suite with at least 80% per-file lines and branches, isolated database tests, browser tests and automatic Cloudflare deployment. Native live checks need the production API to be available and use their own installation credential with fictional data.
...
If an upload times out after Apple accepts it, inspect TestFlight processing before rerunning. The workflow waits up to 30 minutes for processing. A failed check prevents upload. A failed upload never reports successful delivery. Native result bundles are retained for seven days. Web reports belong to the Backend workflow. A successful release-result.json records version, build ID, internal state, source commit and workflow URL for 30 days.
