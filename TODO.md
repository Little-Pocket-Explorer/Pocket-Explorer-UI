...
## Native Social, Profile and Card sharing parity pending macOS qualification (2026-09-17)

- Corrected the native-only routing regression reported from TestFlight: Social no longer replaces Friends, Messages and Shared with Me with one Family Settings card. Each tab now renders its own surface; parent approval remains an action-level requirement for real friend access.
...
- Card Detail now exposes the complete share sheet from both its top-right icon and body action. Share to friend sends a durable idempotent private card-ID message, opens the Messages conversation and lets each side open the card through its authorized collection; it neither creates a public link nor mislabels sharing as a Gift transfer. Share on map retains the approximate-location publication flow, while Copy link alone creates or reuses a revocable public story.
- Profile now also includes the Web's independent Cloud AI permission destination while preserving parent unlock and locked-state withdrawal behavior.
- Added regression coverage for all six Profile destinations, all three Social tabs, the three card-share options, canonical shared-card links, durable message retry, real public-link creation and backend message readback. Swift workspace diagnostics, routing/action assertions, `git diff --check` and ten-catalog parity pass; all catalogs contain 662 unique matching keys.
