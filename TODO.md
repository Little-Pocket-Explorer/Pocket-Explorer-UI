...
- Card Detail now exposes the Web-aligned share sheet from both its top-right icon and body action. Share to friend creates or reuses the revocable single-card public story, sends a durable idempotent shared-card message and opens the Messages conversation; it no longer mislabels sharing as a Gift transfer. Share on map retains the approximate-location publication flow, and Copy link copies the same validated URL.
