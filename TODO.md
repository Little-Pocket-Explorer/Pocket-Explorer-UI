...
- Card Detail now exposes the Web-aligned share sheet from both its top-right icon and body action. Share to friend sends the current verified public card through the existing idempotent gift transfer, Share on map opens the existing approximate-location publication flow, and Copy link creates or reuses the existing revocable single-card public story before copying its URL.
- Added UI regression coverage for all five Profile destinations, all three Social tabs, the three card-share options, real public-link creation and real friend transfer readback. Swift workspace diagnostics, routing/action assertions, `git diff --check` and ten-catalog parity pass; all catalogs contain 660 unique matching keys.
