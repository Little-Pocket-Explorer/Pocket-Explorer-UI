...
@MainActor final class RecallRouteFlowTests: XCTestCase {
    ...
    func testMapCollectionShowsReadyQuizPromptByDefault() {
        ...
        reach(app.buttons["recall-open-\(card.uppercased())"])
    ...
    func testOriginalContextRecallAndColdLinksWithOneStepHome() {
        ...
        let completed = app.segmentedControls["quiz-status"].buttons["Completed 1"]
        completed.tap()
        XCTAssertTrue(completed.isSelected)
        XCTAssertTrue(app.buttons["recall-open-\(card.uppercased())"].exists)
        capture("recall-completed-quiz-keeps-original-discovery")
