...
@MainActor final class SocialFlowTests: XCTestCase {
    ...
    func testJackyJourneyContinuesFromAIThroughMapEventProfileAndExchange() async throws {
        ...
        tap("card-share-options")
        let friendShare = button("card-share-friend")
        XCTAssertTrue(friendShare.waitForExistence(timeout: 5)); XCTAssertTrue(friendShare.isEnabled)
        tap("card-share-friend")
    ...
    func testMapNotificationsAndAudienceScopesUseRealFriendshipAndTransferRecords() async throws {
        enableFriends(mapSharing: true)
        let code = app.staticTexts["friend-code"]; reach(code)
        let request: Friend = try await peer("/api/social/friends", body: ["code": code.label])
        let id = request.id
        app.tabBars.buttons["Map"].tap(); tap("open-notifications")
        let accept = "Accept friendship"
        XCTAssertTrue(button(accept).waitForExistence(timeout: 10))
        _ = try await URLSession.shared.data(from: URL(string: base + "/__fixture/family/offline")!)
        tap(accept)
        XCTAssertTrue(app.staticTexts["notification-error"].waitForExistence(timeout: 10))
        _ = try await URLSession.shared.data(from: URL(string: base + "/__fixture/family/online")!)
        tap(accept)
        let accepted = expectation(for: NSPredicate(format: "exists == false"), evaluatedWith: button(accept))
        await fulfillment(of: [accepted], timeout: 10)
        XCTAssertFalse(app.staticTexts["notification-error"].exists)
        ...
        app.navigationBars.buttons["BackButton"].tap(); tap("open-notifications")
        XCTAssertTrue(button("View Card").waitForExistence(timeout: 15)); tap("View Card")
        XCTAssertTrue(button("discovery-card").waitForExistence(timeout: 5))
        capture("notification-opens-received-gift")
        app.navigationBars.buttons["BackButton"].tap()
        struct Card: Decodable { var id: String }
        let collection: Page<Card> = try await peer("/api/social/friends/\(id)/collection")
        let wanted = try XCTUnwrap(collection.items.first?.id)
        ...
        let declined: Transfer = try await peer("/api/social/friends/\(id)/transfers", body: ["id": UUID().uuidString.lowercased(), "kind": "exchange", "offeredID": seed.cardID, "wantedID": wanted])
        app.navigationBars.buttons["BackButton"].tap(); tap("open-notifications")
        tap("Decline exchange")
        try await waitForTransfer(declined.id, friendID: id, state: "declined")
        capture("notification-exchange-decisions")
        app.navigationBars.buttons["BackButton"].tap()

        struct MapCard: Decodable { var id: String; var audience: String }
        let friendCard: MapCard = try await peer("/api/map-discoveries", body: ["collectibleID": seed.cardID, "audience": "friends_only"])
        app.segmentedControls["map-scope"].buttons["Friends"].tap()
        let friendRow = "map-friend-card-\(friendCard.id)"
        XCTAssertTrue(button(friendRow).waitForExistence(timeout: 10))
        XCTAssertFalse(button("map-shared-\(friendCard.id)").exists)
        tap(friendRow)
        XCTAssertTrue(app.staticTexts["Moon neighbour"].waitForExistence(timeout: 10))
        reach(app.staticTexts["Shared without a location"]); capture("friend-card-without-map-location")
        tap("Done")
        let _: MapCard = try await peer("/api/map-discoveries", body: ["collectibleID": seed.cardID, "audience": "public_approximate", "location": ["latitude": -33.871373, "longitude": 151.212548]])
        app.segmentedControls["map-scope"].buttons["Nearby"].tap()
        PitchFixtureServer.setEventLocation(); tap("map-location"); app.allowLocationIfRequested()
        let event = "map-nearby-event-77777777-7777-4777-8777-777777777777"
        XCTAssertTrue(button(event).waitForExistence(timeout: 15)); tap(event)
        XCTAssertTrue(app.staticTexts["Sky watchers"].waitForExistence(timeout: 10))
        app.navigationBars.buttons["BackButton"].tap()
        tap("map-nearby-card-\(friendCard.id)")
        XCTAssertTrue(app.staticTexts["Moon neighbour"].waitForExistence(timeout: 10)); tap("Done")
        tap("open-notifications"); tap("notification-event-77777777-7777-4777-8777-777777777777")
        XCTAssertTrue(app.staticTexts["Sky watchers"].waitForExistence(timeout: 10))
        app.navigationBars.buttons["BackButton"].tap(); app.navigationBars.buttons["BackButton"].tap()
        app.segmentedControls["map-scope"].buttons["Me"].tap()
        tap("expand-trips"); tap("expand-trips")
        tap("map-share-menu"); tap("Moon neighbour")
        for audience in ["friends_only", "friends_approximate", "public", "public_approximate"] {
            XCTAssertTrue(button("map-audience-\(audience)").exists)
            tap("map-audience-\(audience)")
        }
        tap("map-audience-public"); tap("publish-map-card")
        XCTAssertTrue(app.staticTexts["map-published"].waitForExistence(timeout: 10) || app.otherElements["map-published"].exists)
        let (bytes, response) = try await URLSession.shared.data(from: URL(string: base + "/api/map-discoveries/public")!)
        XCTAssertEqual((response as? HTTPURLResponse)?.statusCode, 200)
        let data = try XCTUnwrap(JSONSerialization.jsonObject(with: bytes) as? [String: Any])
        let cards = try XCTUnwrap(data["items"] as? [[String: Any]])
        let published = try XCTUnwrap(cards.first { $0["audience"] as? String == "public" })
        XCTAssertTrue(published["location"] is NSNull)
        capture("public-card-without-location")
        tap("revoke-map-card")
