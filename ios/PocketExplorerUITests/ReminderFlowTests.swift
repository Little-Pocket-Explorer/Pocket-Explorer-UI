...
final class ReminderFlowTests: XCTestCase {
    ...
    func testWrongAnswerExplainsTheIdeaAndKeepsTheCardAfterRelaunch() {
        ...
        XCTAssertTrue(app.segmentedControls["quiz-status"].buttons["Ready 0"].waitForExistence(timeout: 5))
        XCTAssertFalse(app.buttons["recall-open-\(card)"].exists)
        scrollTo(app.buttons["practice-open-\(card)"])
        capture("quiz-answer-persists-with-optional-practice")
        app.buttons["Done"].tap()
        app.navigationBars.buttons["BackButton"].tap(); app.buttons["open-collection"].tap()
