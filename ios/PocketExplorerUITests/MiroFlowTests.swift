...
final class MiroFlowTests: XCTestCase {
    ...
    func testMiroHomeAndMapCollectionHaveVisiblePrimaryActions() {
        ...
        XCTAssertTrue(app.buttons["map-location"].exists)
        XCTAssertTrue(app.buttons["map-share-menu"].exists)
        XCTAssertTrue(app.buttons["open-notifications"].exists)
        let scopes = app.segmentedControls["map-scope"]
        XCTAssertTrue(scopes.waitForExistence(timeout: 5))
        XCTAssertTrue(scopes.buttons["Me"].exists)
        XCTAssertTrue(scopes.buttons["Nearby"].exists)
        XCTAssertTrue(scopes.buttons["Friends"].exists)
        scopes.buttons["Nearby"].tap()
        XCTAssertTrue(app.buttons["refresh-nearby"].waitForExistence(timeout: 5))
        scopes.buttons["Me"].tap()
        capture("miro-map")
