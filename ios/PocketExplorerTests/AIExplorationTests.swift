...
final class AIExplorationTests: XCTestCase {
    ...
    func testQuestionAnswerCardArtworkQuizAndSharedMetadataSurviveIndependentReloads() throws {
        ...
        XCTAssertNotNil(card.quizAnsweredAt)
        XCTAssertFalse(ReminderPolicy.isEligible(card, now: card.createdAt.addingTimeInterval(172800)))
        XCTAssertEqual(reopened.state.discoveries.count, 1, "A wrong quiz answer must keep the card.")
        XCTAssertEqual(try Data(contentsOf: reopened.mediaURL(card.artworkFilename!)), Data([3, 4]))
        let story = PublicStory.make(trip: reopened.state.trips[0], discoveries: [card])
