...
struct RootView: View {
    ...
    var body: some View {
        TabView(selection: $navigation.tab) {
            ...
            stack(.memories) { MemoriesView(store: store) }
                .tabItem { Label("Memories", systemImage: "sparkles.tv.fill") }.tag(ExplorerTab.memories)
            stack(.friends) { FriendsView(store: store) }
                .tabItem { Label("Friends", systemImage: "person.2.fill") }.tag(ExplorerTab.friends)
        }
        .tint(Theme.forest)
        .environment(artwork)
