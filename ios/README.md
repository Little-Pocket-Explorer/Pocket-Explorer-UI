...
The verified local release toolchain is Xcode 27 RC build 27A266a at /Applications/Xcode-27-RC.app. The project also requires XcodeGen 2.46.0 and targets iOS 17 or later. Xcode projects are generated from project.yml and are not committed. Do not use a GitHub iOS runner during the current quota restriction.
...
Run these commands from the repository root in two separate terminals. Both loopback services avoid upstream AI calls. If this project's fixtures already own the default ports, reuse them instead of starting a second server on those ports.
...
The AI fixture uses 4197 and the sharing-reentry fixture uses 4201. Stop only the services you started when finished. Independent UI runs need separate simulators, DerivedData and mutable fixtures. Disable parallel testing by default.

First list the installed simulators:

```sh
DEVELOPER_DIR=/Applications/Xcode-27-RC.app/Contents/Developer xcrun simctl list devices available
```
...
This iteration publishes TestFlight locally. See [local release operations](../docs/local-release.md). [GitHub release setup](../docs/github-release.md) remains a reference for a future workflow resumption and does not authorize dispatching the current iOS workflow.
